...
class Mnspdb7_Export_CSV{
    ...
    public function array2csv(array &$array, $df){
        ...
        $heading    = array();
        $unwanted   = array('mnspdb7_file', 'mnspdb7_', 'your-');
    ...
    public function download_csv_file(){
        ...
        if( isset($_REQUEST['csv']) && isset( $_REQUEST['nonce'] ) ){
            ...
            $this->download_send_headers( "mnspdb7-" . date("Y-m-d") . ".csv" );
                ...
                foreach ($results as $result) :
                    ...
                    foreach ($resultTmp as $key => $value):
                        ...
                        if (strpos($key, 'mnspdb7_file') !== false ){
                            $data[$key][$i] = empty( $value ) ? '' : $mnspdb7_dir_url.'/'.$value;
                            continue;
                        }
                        if ( is_array($value) ){
