...
/**
 * MNSPDB7 Admin subpage
 */

if (!defined( 'ABSPATH')) exit;

/**
 * mnspdb7_Wp_List_Table class will create the page to load the table
 */
class Mnspdb7_Wp_Sub_Page
{
    ...
    {
        $ListTable = new Mnspdb7_List_Table();
        ...
        ?>
            <div class="wrap">
                ...
                <form method="post" action="">
                    <?php $ListTable->search_box(__( 'Search', 'contact-form-mnspdb7' ), 'search'); ?>
 ...
 */
class Mnspdb7_List_Table extends WP_List_Table
{
    private $form_post_id;
    ...
    public function get_columns()
    {
        ...
        if( !empty($first_row) ){
            ...
            foreach ($first_row as $key => $value) {
                ...
                if ( $key == 'mnspdb7_status' ) continue;

                if( $rm_underscore ) preg_match('/^_.*$/m', $key, $matches);
                if( ! empty($matches[0]) ) continue;

                $key_val       = str_replace( array('your-', 'mnspdb7_file'), '', $key);
    ...
    public function get_bulk_actions() {
        ...
        return array(
            'read'   => __( 'Read', 'contact-form-mnspdb7' ),
            'unread' => __( 'Unread', 'contact-form-mnspdb7' ),
            'delete' => __( 'Delete', 'contact-form-mnspdb7' )
        );
    ...
    private function table_data()
    {
        ...
        foreach ( $results as $result ) {
            ...
            $link  = "<b><a href=admin.php?page=mnspdb7-list.php&fid=%s&ufid=%s>%s</a></b>";
            if(isset($form_value['mnspdb7_status']) && ( $form_value['mnspdb7_status'] === 'read' ) )
                $link  = "<a href=admin.php?page=mnspdb7-list.php&fid=%s&ufid=%s>%s</a>";
        ...
        if( 'delete' === $action ) {
            ...
            foreach ($form_ids as $form_id):
                ...
                foreach ($result_values as $key => $result) {
                    ...
                    if ( ( strpos($key, 'mnspdb7_file') !== false ) &&
                        ! empty( $result ) && 
                        file_exists($mnspdb7_dirname.'/'.$result) ) {

                        unlink($mnspdb7_dirname.'/'.$result);
                    }
            ...
            foreach ($form_ids as $form_id):
                ...
                $result_value  = $results[0]->form_value;
                $result_values = unserialize( $result_value );
                $result_values['mnspdb7_status'] = 'read';
            ...
            foreach ($form_ids as $form_id):
                ...
                $result_value  = $results[0]->form_value;
                $result_values = unserialize( $result_value );
                $result_values['mnspdb7_status'] = 'unread';
                $form_data = serialize( $result_values );
                $cfdb->query(
                    "UPDATE $table_name SET form_value = '$form_data' WHERE form_id = '$form_id'"
                );
    ...
    protected function bulk_actions( $which = '' ) {
        ...
        echo '<label for="bulk-action-selector-' . esc_attr( $which ) . '" class="screen-reader-text">' . __( 'Select bulk action', 'contact-form-mnspdb7' ) . '</label>';
        echo '<select name="action' . $two . '" id="bulk-action-selector-' . esc_attr( $which ) . "\">\n";
        echo '<option value="-1">' . __( 'Bulk Actions', 'contact-form-mnspdb7' ) . "</option>\n";
        ...
        submit_button( __( 'Apply', 'contact-form-mnspdb7' ), 'action', '', false, array( 'id' => "doaction$two" ) );
        echo "\n";
        ...
        _e( 'Export CSV', 'contact-form-mnspdb7' );
        echo '</a>';
        do_action('mnspdb7_after_export_button');
    }
}