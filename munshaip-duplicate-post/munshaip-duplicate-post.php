...
<?php
/*
 * Plugin Name: Munshaip Duplicate Post
...
add_filter( 'post_row_actions', 'munshaip_duplicate_post_link', 10, 2 );
// for "page" post type
add_filter( 'page_row_actions', 'munshaip_duplicate_post_link', 10, 2 );


function munshaip_duplicate_post_link( $actions, $post ) {
	...
	$url = wp_nonce_url(
		add_query_arg(
			array(
				'action' => 'munshaip_duplicate_post_as_draft',
				'post' => $post->ID,
			),
			'admin.php'
		),
 ...
 */
add_action( 'admin_action_munshaip_duplicate_post_as_draft', 'munshaip_duplicate_post_as_draft' );

function munshaip_duplicate_post_as_draft(){
 ...
 */
add_action( 'admin_notices', 'munshaip_duplication_admin_notice' );

function munshaip_duplication_admin_notice() {
